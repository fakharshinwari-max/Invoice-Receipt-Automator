...
def generate_invoice_pdf(invoice, output_path=None):
    ...
    header_table = Table([[business_info, invoice_title]], colWidths=[3.5*inch, 3.5*inch])
...
def send_reminder_email(invoice, days_overdue=0):
    try:
        ...
        html_body = render_template('emails/reminder_email.html', invoice=invoice, days_overdue=days_overdue)
        msg.html = html_body
        if invoice.pdf_path:
            pdf_full_path = os.path.join(current_app.config['UPLOAD_FOLDER'], invoice.pdf_path.replace('uploads/', ''))
            if os.path.exists(pdf_full_path):
