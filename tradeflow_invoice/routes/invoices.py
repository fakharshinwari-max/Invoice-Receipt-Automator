"""Invoice routes - Core feature of TradeFlow Invoice."""
...
from utils import generate_invoice_pdf, send_invoice_email, log_activity, generate_invoice_number
...
def index():
    ...
    # Get clients and jobs for filters
...
def create():
    ...
    # Populate template choices based on user's trade
    user = current_user
...
def edit(invoice_id):
    ...
    if form.validate_on_submit():
        ...
        log_activity(current_user.id, 'update_invoice', 
                    entity_type='invoice', entity_id=invoice.id)
...
def send(invoice_id):
    ...
    if send_invoice_email(invoice, invoice.client.email):
        ...
        log_activity(current_user.id, 'send_invoice', 
                    entity_type='invoice', entity_id=invoice.id)
...
def delete(invoice_id):
    ...
    log_activity(current_user.id, 'delete_invoice', 
...
@invoices_bp.route('/invoices/<int:invoice_id>/items/add', methods=['POST'])
@login_required
def add_item(invoice_id):
    """Add line item to invoice (AJAX)."""
    invoice = Invoice.query.filter_by(id=invoice_id, user_id=current_user.id).first_or_404()
...
def delete_item(invoice_id, item_id):
    """Delete line item from invoice (AJAX)."""
...
def record_payment(invoice_id):
    ...
    if amount > 0:
        ...
        log_activity(current_user.id, 'record_payment', 
                    entity_type='invoice', entity_id=invoice.id,
...
@invoices_bp.route('/invoices/<int:invoice_id>/remind', methods=['POST'])
@login_required
def send_reminder(invoice_id):
    ...
    if send_reminder_email(invoice, days_overdue):
        ...
        log_activity(current_user.id, 'send_reminder', 
                    entity_type='invoice', entity_id=invoice.id)
